...
if ($exists -and $(Get-Content $path | Out-String) -match "VERSION_FULL (\d+).(\d+).(\d+).(\d+)" -and [int]$matches[1] -gt 0) {
...
if ($version[0] -eq 0) { throw 'ReShade version is unknown. Fetch upstream tags or include the generated res/version.h before building; a zero version breaks shader compatibility.' }
