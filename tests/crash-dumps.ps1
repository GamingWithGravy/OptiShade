...
try{
 ...
 try{
  ...
  Check ($index.Included -eq 0 -and $index.TextReports -eq 4) 'Text summaries generated by default without raw dumps'
  ...
  Check ((@($index.Candidates|Where-Object AnalysisStatus -like 'Partial*')).Count -eq 4) 'Invalid fixture dumps explicitly reported in text'
 }finally{$zip.Dispose()}
 ...
 try{AddCrashDumpsToArchive $zip $stream @($candidate) "$root/small.tmp" 500000 $true}finally{$zip.Dispose();$stream.Dispose()}
