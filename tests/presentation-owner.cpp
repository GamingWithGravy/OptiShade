...
int main(){
    ...
    HWND instrument=CreateWindowExW(0,L"STATIC",L"instrument",WS_POPUP,0,0,400,300,main,nullptr,GetModuleHandleW(nullptr),nullptr);
    HWND tool=CreateWindowExW(WS_EX_TOOLWINDOW,L"STATIC",L"tool",WS_OVERLAPPED,0,0,400,300,nullptr,nullptr,GetModuleHandleW(nullptr),nullptr);
    assert(main&&secondary&&child&&instrument&&tool);
    assert(owner.Accept(nullptr,true));
    assert(!owner.Accept(instrument));
    assert(!owner.Accept(instrument,true));
    assert(!owner.Accept(tool));
    assert(!owner.Accept(tool,true));
    ...
    assert(!owner.Accept(instrument,true,&first));
    assert(!owner.Accept(tool,true,&first));
    assert(owner.Accept(main, true, &other));
