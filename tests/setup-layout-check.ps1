...
foreach($name in @('MsfsCopies','Install','Restore','Runtime','SettingsPage','TroubleshootingNav','TroubleshootingPage','ResetDefaults','ImportZip','CheckUpdates','RecoveryRepair','RecoveryRestore')){if(-not $form.FindName($name)){throw "Missing $name"}}
...
$form.FindName('Hardware').Text="NVIDIA GeForce RTX 5090, AMD Radeon(TM) Graphics | 63 GB RAM | Microsoft Windows 11 Pro"
foreach($width in @(1000,1100)){
 ...
 $homePanel=$form.FindName('HomePage');$dev=$form.FindName('DeveloperMessagePanel');$devPoint=$dev.TranslatePoint([Windows.Point]::new(0,0),$homePanel);if($dev.ActualHeight -lt 40 -or $devPoint.Y+$dev.ActualHeight -gt $homePanel.ActualHeight){throw 'Developer message extends beyond home area'}
