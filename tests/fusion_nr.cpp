// OptiShade integration probe adapted from upstream NR residual smoke test (GPL-3.0).
...
int wmain(int argc,wchar_t** argv) try {
    ...
    HMODULE module=LoadLibraryW(argv[1]); expect(module!=nullptr,"LoadLibrary nvngx.dll"); Sleep(2000);
    ...
    auto completed=proc<unsigned long long(*)()>(module,"OptiShadeNrCompletedFrames");
    ...
    WNDCLASSW wc{};wc.hInstance=GetModuleHandleW(nullptr);wc.lpfnWndProc=DefWindowProcW;wc.lpszClassName=L"OptiShadeNrProbe";RegisterClassW(&wc);
    HWND window=CreateWindowW(wc.lpszClassName,L"NR test",WS_OVERLAPPEDWINDOW,0,0,1280,720,nullptr,nullptr,wc.hInstance,nullptr);
    ComPtr<IDXGIFactory2> f2;check(factory.As(&f2));ComPtr<IDXGISwapChain1> swap;
    DXGI_SWAP_CHAIN_DESC1 sd{};sd.Width=1280;sd.Height=720;sd.Format=DXGI_FORMAT_R8G8B8A8_UNORM;sd.BufferCount=2;sd.SampleDesc.Count=1;sd.BufferUsage=DXGI_USAGE_RENDER_TARGET_OUTPUT;sd.SwapEffect=DXGI_SWAP_EFFECT_FLIP_DISCARD;
    check(f2->CreateSwapChainForHwnd(queue.Get(),window,&sd,nullptr,nullptr,&swap));
    ...
    constexpr UINT w=640,h=360,ow=1280,oh=720;
    ...
    for (int test=0;test<2;++test) {
        ...
        std::printf("Completed neural-rendering frames: %llu\n",completed());
        readback->Unmap(0,nullptr);
    }
    expect(completed()>3,"Neural rendering did not evaluate the model");
    ngx(release(feature)); ngx(release(other)); ngx(destroy(p)); ngx(destroy(p2)); CloseHandle(event);
    std::puts("PASS: real upscaling and NVIDIA neural model evaluations completed; finite GPU readbacks");
    return 0;
